...
#define OS_INT_DisableAll()    { __asm volatile ("csrci mstatus,8 \n\t" : : : "memory" ); }  // Clear (global) machine interrupt enable bit in mstatus register
#define OS_INT_EnableAll()     { __asm volatile ("csrsi mstatus,8 \n\t" : : : "memory" ); }  // Set   (global) machine interrupt enable bit in mstatus register
#define OS_INT_PreserveAll(p)  { __asm volatile ("csrr t0, mstatus      \n\t" \
                                                 "andi t0, t0, 8        \n\t" \
                                                  OS__STORE " t0, 0(%0) \n\t" \
                                                 :                            \
                                                 : "r" (p)                    \
...
#define OS_INT_RestoreAll(p)   { __asm volatile ( OS__LOAD " t0, 0(%0)  \n\t" \
                                                 "csrs  mstatus, t0     \n\t" \
                                                 :                            \
                                                 : "l" (p)                    \
                                                 : "t0", "memory"             \
...
#define OS_U64   unsigned long long
#define OS_UINT  unsigned int
#define OS_BOOL  OS_U8

#if   (__riscv_xlen == 32)
  #define OS__STORE     "sw"
  #define OS__LOAD      "lw"
  #define OS__REGBYTES  4
  #define OS_REG_TYPE   OS_U32
#elif (__riscv_xlen == 64)
  #define OS__STORE     "sd"
  #define OS__LOAD      "ld"
  #define OS__REGBYTES  8
  #define OS_REG_TYPE   OS_U64
#endif
...
typedef struct {
  OS_U32      Counters;
  OS_REG_TYPE OS_REG_S0;
  OS_REG_TYPE OS_REG_S1;
  OS_REG_TYPE OS_REG_S2;
  OS_REG_TYPE OS_REG_S3;
  OS_REG_TYPE OS_REG_S4;
  OS_REG_TYPE OS_REG_S5;
  OS_REG_TYPE OS_REG_S6;
  OS_REG_TYPE OS_REG_S7;
  OS_REG_TYPE OS_REG_S8;
  OS_REG_TYPE OS_REG_S9;
  OS_REG_TYPE OS_REG_S10;
  OS_REG_TYPE OS_REG_S11;
  OS_REG_TYPE OS_REG_PC;
} OS_REGS_BASE;
...
typedef struct {
  OS_REG_TYPE OS_REG_PC;
  OS_REG_TYPE OS_REG_SP;
  OS_REG_TYPE OS_REG_S0;
  OS_REG_TYPE OS_REG_S1;
  OS_REG_TYPE OS_REG_MIE;
  OS_REG_TYPE OS_REG_MSTATUS;
...
#endif
#if (OS_CPU_HAS_VFP == 0)
  OS_REG_TYPE OS_REG_FS0;
  OS_REG_TYPE OS_REG_FS1;
  OS_REG_TYPE OS_REG_FS2;
  OS_REG_TYPE OS_REG_FS3;
  OS_REG_TYPE OS_REG_FS4;
  OS_REG_TYPE OS_REG_FS5;
  OS_REG_TYPE OS_REG_FS6;
  OS_REG_TYPE OS_REG_FS7;
  OS_REG_TYPE OS_REG_FS8;
  OS_REG_TYPE OS_REG_FS9;
  OS_REG_TYPE OS_REG_FS10;
  OS_REG_TYPE OS_REG_FS11;
#endif
  void*       pBuffer;
  OS_U32      BufferSize;
} OS_MAIN_CONTEXT;
...
OS_REG_TYPE OS_GetHartID(void) OS_TEXT_SECTION_ATTRIBUTE(OS_GetHartID);
...
typedef void OS_IRQ_HANDLER_EX(void* pContext);

typedef struct {
  OS_IRQ_HANDLER_EX *(pfISR);
  void*             pContext;
} OS_IRQ_HANDLER_CONTEXT;

OS_REG_TYPE OS_TrapHandler(OS_REG_TYPE mcause, OS_REG_TYPE mepc)  OS_TEXT_SECTION_ATTRIBUTE(OS_TrapHandler);

/****** RISC-V Core-local interrupt handling ***************************/
#if (__riscv_xlen == 32)
  #define MCAUSE_INT       (0x80000000u)
  #define MCAUSE_CAUSE     (0x7FFFFFFFu)
#else
  #define MCAUSE_INT       (0x8000000000000000u)
  #define MCAUSE_CAUSE     (0x7FFFFFFFFFFFFFFFu)
#endif
#define   ECLIC_EXC_CAUSE  (0x7FFu)
...
void            OS_CLINT_ClearIntPending(CLINT_IRQn IRQIndex)                                                                   OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_ClearIntPending);
void            OS_CLINT_DisableInt     (CLINT_IRQn IRQIndex)                                                                   OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_DisableInt);
void            OS_CLINT_EnableInt      (CLINT_IRQn IRQIndex)                                                                   OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_EnableInt);
OS_BOOL         OS_CLINT_GetIntPending  (CLINT_IRQn IRQIndex)                                                                   OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_GetIntPending);
void            OS_CLINT_Init           (OS_U8 NumInterrupts, OS_IRQ_HANDLER* apfISR[])                                         OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_Init);
OS_IRQ_HANDLER* OS_CLINT_InstallISR     (CLINT_IRQn IRQIndex, OS_IRQ_HANDLER* pfISR)                                            OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_InstallISR);
void            OS_CLINT_SetIntPending  (CLINT_IRQn IRQIndex)                                                                   OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_SetIntPending);
void            OS_CLINT_SetDirectMode  (void)                                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_SetDirectMode);
void            OS_CLINT_SetVectoredMode(void)                                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_CLINT_SetVectoredMode);

/****** RISC-V Platform-level interrupt handling ***********************/
OS_U32          OS_PLIC_ClaimInt        (void)                                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_ClaimInt);
void            OS_PLIC_CompleteInt     (OS_U32 IRQIndex)                                                                       OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_CompleteInt);
void            OS_PLIC_DisableInt      (OS_U32 IRQIndex)                                                                       OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_DisableInt);
void            OS_PLIC_EnableInt       (OS_U32 IRQIndex)                                                                       OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_EnableInt);
OS_U32          OS_PLIC_GetIntPriority  (OS_U32 IRQIndex)                                                                       OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_GetIntPriority);
OS_U32          OS_PLIC_GetIntThreshold (void)                                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_GetIntThreshold);
void            OS_PLIC_Init            (OS_U32 BaseAddr, OS_U16 NumInterrupts, OS_U32 NumPriorities, OS_IRQ_HANDLER* apfISR[]) OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_Init);
OS_IRQ_HANDLER* OS_PLIC_InstallISR      (OS_U32 IRQIndex, OS_IRQ_HANDLER* pfISR)                                                OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_InstallISR);
OS_U32          OS_PLIC_SetIntPriority  (OS_U32 IRQIndex, OS_U32 Priority)                                                      OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_SetIntPriority);
void            OS_PLIC_SetIntThreshold (OS_U32 Threshold)                                                                      OS_TEXT_SECTION_ATTRIBUTE(OS_PLIC_SetIntThreshold);

/****** NucleiSys Enhanced core-local interrupt handling ***************/
void            OS_ECLIC_DisableInt     (OS_UINT IRQIndex)                                                                      OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_DisableInt);
void            OS_ECLIC_EnableInt      (OS_UINT IRQIndex)                                                                      OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_EnableInt);
OS_U8           OS_ECLIC_GetNumLevelBits(void)                                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_GetNumLevelBits);
OS_U8           OS_ECLIC_GetIntPriority (OS_UINT IRQIndex)                                                                      OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_GetIntPriority);
OS_U8           OS_ECLIC_GetIntThreshold(void)                                                                                  OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_GetIntThreshold);
void            OS_ECLIC_Init           (void* pBaseAddr, void* pVectorTable, void* pTrapHandler)                               OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_Init);
void            OS_ECLIC_SetNumLevelBits(OS_U8 NumLevelBits)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_SetNumLevelBits);
void            OS_ECLIC_SetIntPriority (OS_UINT IRQIndex, OS_U8 InterruptPriority)                                             OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_SetIntPriority);
void            OS_ECLIC_SetIntThreshold(OS_U8 Threshold)                                                                       OS_TEXT_SECTION_ATTRIBUTE(OS_ECLIC_SetIntThreshold);

/****** Lattice programmable interrupt handling ************************/
typedef enum {
   IRQ_S0 = 0u
  ,IRQ_S1 = 1u
  ,IRQ_S2 = 2u
...
typedef enum {
   PIC_INTPOLARITY_HIGH = 0u
  ,PIC_INTPOLARITY_LOW  = 1u
} PIC_INTPOLARITY;

void               OS_PIC_ClearIntPending(PIC_IRQn IRQIndex)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_ClearIntPending);
void               OS_PIC_DisableInt     (PIC_IRQn IRQIndex)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_DisableInt);
void               OS_PIC_EnableInt      (PIC_IRQn IRQIndex)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_EnableInt);
OS_BOOL            OS_PIC_GetIntPending  (PIC_IRQn IRQIndex)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_GetIntPending);
PIC_INTPOLARITY    OS_PIC_GetIntPolarity (PIC_IRQn IRQIndex)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_GetIntPolarity);
void               OS_PIC_Init           (OS_U32 BaseAddr, OS_U8 NumInterrupts, OS_IRQ_HANDLER* apfISR[])                       OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_Init);
void               OS_PIC_Init_Ex        (OS_U32 BaseAddr, OS_U8 NumInterrupts, OS_IRQ_HANDLER_CONTEXT* apfISR[])               OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_Init_Ex);
OS_IRQ_HANDLER*    OS_PIC_InstallISR     (PIC_IRQn IRQIndex, OS_IRQ_HANDLER* pfISR)                                             OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_InstallISR);
OS_IRQ_HANDLER_EX* OS_PIC_InstallISR_Ex  (PIC_IRQn IRQIndex, OS_IRQ_HANDLER_EX* pfISR, void* pContext)                          OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_InstallISR_Ex);
void               OS_PIC_SetIntPending  (PIC_IRQn IRQIndex)                                                                    OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_SetIntPending);
void               OS_PIC_SetIntPolarity (PIC_IRQn IRQIndex, PIC_INTPOLARITY Polarity)                                          OS_TEXT_SECTION_ATTRIBUTE(OS_PIC_SetIntPolarity);
...
/*********************************************************************
*
*       Custom OS_INT_LeaveIntStack
*
*  Additional information
*    Used to prevent compilers from doing tail call optimization
*    when OS_INT_LeaveIntStack() is called at the end of a function.
*    Tail call optimization would move stack operations that should
*    operate on the sys-stack to the end where we are already using
*    the task stack.
*/
void OS__LeaveIntStack(void) OS_TEXT_SECTION_ATTRIBUTE(OS__LeaveIntStack);
#define OS_INT_LeaveIntStack() {OS_INT_Disable(); OS__LeaveIntStack(); __asm volatile ("nop"); }
